...
//!   ("reg", endpoint_id)        → (owner, payTo, price_stroops, is_ai_powered)
...
pub struct EndpointRecord {
    ...
    pub name:           String,
    pub active:         bool,
    pub is_ai_powered:  bool,   // true when the endpoint is an AI agent (agent sells to agent)
...
impl EndpointRegistry {
    ...
    /// `is_ai_powered` marks endpoints whose responses are generated by
    /// an AI model — agents can filter for or prefer these in the catalog.
    pub fn register(
        env:           Env,
        ...
        // is_ai_powered is included so off-chain consumers can index AI endpoints
        // without querying contract state.
        env.events().publish(
            (symbol_short!("reg"), endpoint_id),
            (owner, pay_to, price_stroops, name, is_ai_powered),
        );
    }
